...
class FullScreenImageViewer:
    def __init__(self, folder_to_watch, interval=5):
        ...
        self.transition_frames = 30  # Number of frames for 1-second transition at 30 fps
    ...
    def display_image(self):
        ...
        current_image = Image.open(self.images[self.current_image_index])
        next_index = (self.current_image_index + 1) % len(self.images)
        next_image = Image.open(self.images[next_index])

        screen_width = self.root.winfo_screenwidth()
        screen_height = self.root.winfo_screenheight()

        current_image = current_image.resize((screen_width, screen_height), Image.LANCZOS)
        next_image = next_image.resize((screen_width, screen_height), Image.LANCZOS)
        ...
        for i in range(self.transition_frames + 1):
            ...
            self.root.update()
            time.sleep(1 / self.transition_frames)
